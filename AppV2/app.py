...
def download_latest_answer_as_docx():
    if st.session_state.chat_history:
        latest_answer = st.session_state.chat_history[-1].content
        doc = Document()
        doc.add_paragraph(latest_answer)
        doc_io = BytesIO()
        doc.save(doc_io)
        doc_io.seek(0)
        st.download_button(
            label="Download Latest Answer as DOCX",
            data=doc_io,
            file_name="latest_answer.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        )
    else:
        st.warning("No chat history available. Please initiate a conversation first.")
...
def main():
    ...
    st.title("Resume Optimizer :books:")
    ...
    if st.session_state.get("pdf_processed"):
        ...
        if send_button and user_question:
            ...
            download_latest_answer_as_docx()
    else: 
